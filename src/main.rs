...
fn main() {
    ...
    let new_profile = Profile::build_from_text(text);
    new_profile.save_to_file("foo.json");

    let loaded_profile = Profile::load_from_file("foo.json");
    println!("Ranks of each ngram:");
    for (ngram, rank) in loaded_profile.ngram_ranks {
        println!("{}: {}", ngram, rank);
    }
}
